...
**Domain:** `HR / Performance` · **Suite:** `HR & Recruiting Suite` · **Accent:** `#fbbf24`
...
**Performance Review Ai** is a standalone, browser-based AI workflow app for **HR / Performance**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `feedback_notes`<br>Feedback notes | textarea | Yes | — | Affects input: Feedback notes. |
...
| `company_rubric`<br>company rubric | text | No | — | Affects customization: company rubric. |
| `review_cycle`<br>review cycle | text | No | — | Affects customization: review cycle. |
| `role_level`<br>role level | select | No | — | Affects customization: role level. |
| `tone`<br>tone | text | No | — | Affects customization: tone. |
| `calibration_strictness`<br>calibration strictness | slider | No | 50 | Affects customization: calibration strictness. |
| `competency_weights`<br>competency weights | text | No | — | Affects customization: competency weights. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9144
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9144`. |
...
```bash
curl http://127.0.0.1:9144/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9144/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9144 uvicorn app.main:app --host 0.0.0.0 --port 9144
```
...
- `plugins/performance-review-ai.json` for plugin metadata.
